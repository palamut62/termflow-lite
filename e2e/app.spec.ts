...
test('shows agent work details below provider terminals', async () => {
  ...
  await app.evaluate(({ ipcMain }) => {
    ...
    ipcMain.handle('pty:create', () => ({ pid: 123 }))
  })
  const tabCountBeforeHandover = await win.locator('.tab').count()
  await agentProfile.selectOption('codex')
  await expect(win.getByRole('dialog', { name: 'Review agent handover' })).toBeVisible()
  await expect(win.locator('.tab')).toHaveCount(tabCountBeforeHandover)
  await win.getByRole('button', { name: 'Start handover' }).click()
...
test('configures agent security and opens the agent inbox', async () => {
  ...
  const mode = win.locator('.settings-field', { hasText: 'Default permission mode' }).locator('select')
  await mode.selectOption('full')
  await win.keyboard.press('Escape')
  // Existing tabs retain their launch policy even after the default changes.
  await expect(securityProfile).toBeVisible()
  await win.click('.new-tab-btn')
